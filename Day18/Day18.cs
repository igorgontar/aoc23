...
class Program
{
    public static void Main(string[] args)
    {
        try
        {
            ...
            Profiler.Warn("=== AoC 2023 Day18 ===");

            using(Profiler.CheckPoint("Puzzle1"))
            {
                using(Profiler.CheckPoint("Test"))
                    Profiler.Trace("Result  : {0} (62)", Puzzle1.Solve("inputTest.txt"));
                
                using(Profiler.CheckPoint("Input"))
                    Profiler.Trace("Result: {0} (46394)", Puzzle1.Solve("input.txt"));
            }        
            
            // using(Profiler.CheckPoint("Puzzle2"))
            // {
            //     using(Profiler.CheckPoint("Test"))
            //         Profiler.Trace("Result  : {0} ()", Puzzle2.Solve("inputTest3.txt"));
